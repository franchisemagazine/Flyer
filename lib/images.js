...
export async function findExactWebProductImage(model){
  ...
  const candidates=extractWebImageCandidates(search.bytes.toString('utf8'))
  ...
  for(const candidate of candidates){
    ...
    try{
      ...
      const preferredImage=findPageImage(html);
      const imageOptions=[preferredImage,candidate.imageUrl].filter(Boolean);
      for(const possible of imageOptions){
        const imageUrl=await safePublicURL(possible);
...
export async function boundedFetch(url, options = {}) {
  ...
  const response = await fetch(target, {...rest, redirect:'error', signal:AbortSignal.timeout(20000)});
