...
export function validateFields(fields, catalog) {
  ...
  if (!validSelectionArray(fields.model,allowedModels)) errors.model = 'Choose one or more models in the selected category.';
  if (!validSelectionArray(fields.condition,catalog.conditions)) errors.condition = 'Choose one or more conditions from the list.';
